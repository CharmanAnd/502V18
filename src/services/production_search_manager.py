...
class ProductionSearchManager:
    ...
    def search_with_fallback(self, query: str, max_results: int = 10) -> List[SearchResult]:
        ...
        if cached_results:
            ...
            # Salva uso de cache
            salvar_etapa("cache_hit", {"query": query, "results_count": len(cached_results)}, categoria="pesquisa_web")
        ...
        salvar_etapa("busca_iniciada", {"query": query, "max_results": max_results}, categoria="pesquisa_web")
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:  # Reduz workers
            ...
            for future in as_completed(future_to_provider, timeout=60):
                ...
                try:
                    ...
                    if results:
                        # Salva resultados de cada provedor imediatamente
                        salvar_etapa(f"resultados_{provider_name}", {
                    ...
                    salvar_erro(f"busca_{provider_name}", e, contexto={"query": query})
                    self._handle_provider_error(provider_name, e)

        # Aplica filtros de URL para remover conteúdo irrelevante
        logger.info(f"🔍 Aplicando filtros de URL em {len(all_results)} resultados...")
        filtered_results = url_filter_manager.filtrar_lista_urls(all_results)
        ...
        if dict_results:
            ...
            salvar_etapa("busca_final", {
                "query": query,
                "providers_used": successful_providers,
                "total_results": len(dict_results),
                "results": dict_results
            }, categoria="pesquisa_web")

        logger.info(f"🎯 Busca final: {len(dict_results)} resultados únicos e filtrados de {len(successful_providers)} provedores")
