...
class VisualProofsGenerator:
    ...
    def generate_complete_proofs_system(
        ...
        try:
            ...
            if not selected_experiments:
                ...
                # Usa experimentos padrão em vez de falhar
                logger.warning("🔄 Usando experimentos padrão como fallback")
                selected_experiments = self._get_default_experiments(context_data)
            ...
            for experiment in selected_experiments:
                try:
                    ...
                    if customized_proof and self._validate_proof(customized_proof):
                        ...
                        # Salva cada prova customizada imediatamente
            ...
            if not customized_proofs:
                ...
                customized_proofs = self._get_default_visual_proofs(context_data)
            ...
            # Retorna provas básicas em vez de falhar
            logger.warning("🔄 Retornando provas visuais básicas")
            return self._get_default_visual_proofs(context_data)
    
    def _get_default_experiments(self, context_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Retorna experimentos padrão como fallback"""
        ...
        return [
            {
                ...
                'conceito': 'Mostrar situação atual vs ideal',
                'experimento': f'Comparação visual entre situação atual em {segmento} e potencial máximo',
                'materiais': ['Gráficos comparativos', 'Dados visuais', 'Projeções'],
                'impacto': 'Alto'
            },
            {
                'nome': 'Prova Social Visual',
                'conceito': 'Mostrar resultados de outros',
                'experimento': f'Cases de sucesso visuais de profissionais de {segmento}',
                'materiais': ['Screenshots', 'Depoimentos', 'Números'],
                'impacto': 'Alto'
            },
            {
                'nome': 'Urgência Temporal',
                'conceito': 'Mostrar custo do tempo',
                'experimento': f'Calculadora visual do custo de cada mês sem otimizar {segmento}',
                'materiais': ['Calculadora', 'Gráfico temporal', 'Valores'],
                'impacto': 'Médio'
            }
        ]
    ...
    def _get_default_visual_proofs(self, context_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        return [
            {
                'nome': f'Transformação {segmento}',
                'conceito_alvo': f'Mostrar potencial de transformação em {segmento}',
                'experimento': f'Demonstração visual do antes e depois em {segmento}',
                'materiais': ['Gráficos', 'Comparações', 'Dados visuais'],
                'roteiro_completo': f'Mostrar situação atual vs potencial em {segmento}',
                'impacto_esperado': 'Alto',
                'momento_ideal': 'Durante apresentação principal'
            },
            {
                'nome': 'Prova Social Numérica',
                'conceito_alvo': 'Validar eficácia através de números',
                'experimento': f'Apresentação de resultados numéricos em {segmento}',
                'materiais': ['Estatísticas', 'Gráficos', 'Comparações'],
                'roteiro_completo': 'Mostrar dados concretos de resultados',
                'impacto_esperado': 'Alto',
            ...
            },
            {
                ...
                'experimento': f'Cálculo visual do custo de não otimizar {segmento}',
                'materiais': ['Calculadora', 'Projeções', 'Comparações'],
                'roteiro_completo': 'Demonstrar perda financeira da inação',
